...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-b5b8af44";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-b5b8af44">'+'<span class="tile-face-b5b8af44 tile-front-b5b8af44"></span>'+'<span class="tile-face-b5b8af44 tile-back-b5b8af44"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-b5b8af44")){return;}
var back=tile.querySelector(".tile-back-b5b8af44");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-b5b8af44");if(isMine){tile.classList.add("boom-b5b8af44");}
if(dim){tile.classList.add("dimmed-b5b8af44");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-b5b8af44")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-b5b8af44");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-b5b8af44")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-b5b8af44");void panelEl.offsetWidth;panelEl.classList.add("jolt-b5b8af44");}
endRound();}
